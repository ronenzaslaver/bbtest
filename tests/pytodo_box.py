...
class PyToDoBox(BlackBox):
    ...
    def install(self):
        ...
        self.host.put(os.path.join(src_dir, 'todo.py'),
                      remote=self.host.join(dest_dir, 'todo.py'))

        if self.host.os == 'linux':
            self.host.run('chmod', '777', self.host.join(self.home, "todo.py"))

    def run(self, *args):
        todo_py = self.host.join(self.home, 'todo.py')
